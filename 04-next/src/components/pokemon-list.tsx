...
export function PokemonList({ pokemons, loading, searchResults, error, errorMessage }: Props) {
	if (loading) {
		return (
	...
	if (error) {
		return (
			<div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
				<p className="col-span-full rounded-lg border border-red-400 bg-red-100 p-4 text-center text-red-700">
					Ocurrió un error al buscar los Pokémon: {errorMessage}
				</p>
			</div>
		)
	}
